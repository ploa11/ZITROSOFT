...
Public Class Form_Reg_RQ
    ...
    Private Sub Button10_Click(sender As Object, e As EventArgs) Handles Button10.Click
        Form_rev_rq.busq = "busqueda"
        Form_rev_rq.Label2.Enabled = False
        Form_rev_rq.Button1.Enabled = False
        Form_rev_rq.Show()
        'llenar_PRO()
        'Button9.Enabled = True
    ...
    Private Sub Button11_Click(sender As Object, e As EventArgs) Handles Button11.Click
        ...
        Dim cod_editar, item_rq_editar, cod_rq_editar, nom_produ_edit, unidad_rq_editar, medida, marca, color, a_lima, priori, estado, fec_reg_edit, obs, estado_edit, descrip_editar As String
        Dim cant_EDITAR, strok, prec_unit_editar, prec_total_editar, igv_editar As Integer
        accion = "guardar"
        For i As Integer = 0 To dgv.Rows.Count - 1
            cod_editar = Trim(dgv.Rows(i).Cells(0).Value)
            item_rq_editar = (dgv.Rows(i).Cells(1).Value)
            cod_rq_editar = Trim(dgv.Rows(i).Cells(2).Value)
            nom_produ_edit = Trim(dgv.Rows(i).Cells(3).Value)
            unidad_rq_editar = Trim(dgv.Rows(i).Cells(4).Value)
            medida = Trim(dgv.Rows(i).Cells(5).Value)
            marca = Trim(dgv.Rows(i).Cells(6).Value)
            color = Trim(dgv.Rows(i).Cells(7).Value)
            a_lima = Trim(dgv.Rows(i).Cells(8).Value)
            cant_EDITAR = (dgv.Rows(i).Cells(9).Value)
            strok = (dgv.Rows(i).Cells(10).Value)
            priori = Trim(dgv.Rows(i).Cells(11).Value)
            obs = (dgv.Rows(i).Cells(12).Value)
            fec_reg_edit = Trim(dgv.Rows(i).Cells(13).Value)
            estado_edit = (dgv.Rows(i).Cells(14).Value)
            ...
            sql = "UPDATE P_REQUERIMIENTO SET CANT='" & UCase(cant_EDITAR) & "', item= '" & UCase(item_rq_editar) & "', nom_prod='" & nom_produ_edit & "', unid='" & unidad_rq_editar & "', MARCA= '" & marca & "', color='" & color & "', medid='" & medida & "', ESTADO='" & estado_edit & "' WHERE COD='" & cod_editar & "'"
            Form_Reg_SRV_SQL.conectar()
            com = New SqlClient.SqlCommand(sql, Form_Reg_SRV_SQL.conexion)
    ...
    Private Sub Button14_Click(sender As Object, e As EventArgs) Handles Button14.Click
        ...
        Try
            ...
            If savefiledialog1.ShowDialog = DialogResult.OK Then
                ...
                xlwb = Nothing
                xlapp.quit()
                MsgBox("EXPORTADO CORRECTAMENTE", MsgBoxStyle.Information, "ZITRO")
        ...
        Try
            For o = 0 To ListView1.Items.Count - 1
                If accion = "guardar" Then
                    sql = "select *from P_REQUERIMIENTO where  COD='" + TextBox3.Text + "'"
                    Form_Reg_SRV_SQL.conectar()
                    com = New SqlClient.SqlCommand(sql, Form_Reg_SRV_SQL.conexion)
                    dr = com.ExecuteReader
                    If dr.Read Then
    ...
    Private Sub Button18_Click(sender As Object, e As EventArgs) Handles Button18.Click
        Try
            ...
            Process.Start("explorer.exe", (Form_Reg_SRV_SQL.NOM_CARP_RQ & n_carp))
        Catch ex As Exception
    ...
    Private Sub Button19_Click(sender As Object, e As EventArgs) Handles Button19.Click
        ...
        Form_Orden_Compra.TextBox7.Text = TextBox7.Text
        Form_Orden_Compra.TextBox8.Text = TextBox5.Text
    ...
    Private Sub EDITA_RQ_GERENCIA()
        sql = "UPDATE T_REQUERIMIENTO SET DETA_APROB_USU='" & UCase(ComboBox6.Text) & "', ESTADO= '" & UCase(ComboBox6.Text) & "', F_APROB='" & DateTimePicker4.Value.ToString("yyyyMMdd") & "' WHERE COD='" & TextBox1.Text & "'"
        Form_Reg_SRV_SQL.conectar()
        com = New SqlClient.SqlCommand(sql, Form_Reg_SRV_SQL.conexion)
        res = com.ExecuteNonQuery
        Form_Reg_SRV_SQL.conexion.Close()
    ...
    Public Sub buscar_rq2()
        'Dim aum_cod As String
        ' Dim dat As String = "RQ"
        'Dim nc As String = Form_rev_rq.dgv.Rows(dgv.CurrentRow.Index).Cells(0).Value
        sql = "Select *from T_REQUERIMIENTO where cod='" + Form_rev_rq.nct + "'"
        Form_Reg_SRV_SQL.conectar()
        com = New SqlClient.SqlCommand(sql, Form_Reg_SRV_SQL.conexion)
        dr = com.ExecuteReader
        If dr.Read Then
            ...
            Form_Orden_Compra.TextBox1.Text = dr(0)
            Form_Orden_Compra.TextBox7.Text = dr(4)
            Form_Orden_Compra.TextBox8.Text = dr(1)
    ...
    Private Sub crear_carpeta()
        Try
            ...
            'Directory.CreateDirectory("\\server\SISTEMA\RQ\" & n_carp)
            Directory.CreateDirectory(Form_Reg_SRV_SQL.NOM_CARP_RQ & n_carp)
    ...
    Private Sub copiar2()
        Try
            ...
            Dim copia As String
            n_carp = UCase(TextBox1.Text)
            ruta = Form_Reg_SRV_SQL.CARP_SERV & "RUMISOFT2019\RUMISOFT\bin\Debug\REQUERIMIENTO.xlsx"
            copia = Form_Reg_SRV_SQL.NOM_CARP_RQ & n_carp & "\" & n_carp & ".xlsx"
    ...
    Private Sub borrar()
        Try
            ...
            ArchivoBorrar = Form_Reg_SRV_SQL.CARP_SERV & "RUMISOFT2019\RUMISOFT\bin\Debug\REQUERIMIENTO.xlsx"
    ...
    Private Sub copiar_ruta1()
        ...
        Try
            ...
            'ruta = "\\server\SISTEMA\RUMISOFT2019\RUMISOFT\bin\Debug\formato\REQUERIMIENTO.xlsx"
            'copia = "\\server\SISTEMA\RUMISOFT2019\RUMISOFT\bin\Debug\REQUERIMIENTO.xlsx"
            ruta = Form_Reg_SRV_SQL.CARP_SERV & "RUMISOFT2019\RUMISOFT\bin\Debug\formato\REQUERIMIENTO.xlsx"
            copia = Form_Reg_SRV_SQL.CARP_SERV & "RUMISOFT2019\RUMISOFT\bin\Debug\REQUERIMIENTO.xlsx"
            IO.File.Copy(ruta, copia)
        ...
        Try
            ...
            If OpenFileDialog1.ShowDialog() = DialogResult.OK Then
                ...
                If extension = ".jpg" Or extension = ".png" Or extension = ".gif" Or extension = ".pdf" Or extension = ".doc" Or extension = ".xls" Or extension = ".ppt" Or extension = ".docx" Or extension = ".xlsx" Or extension = ".xlsm" Or extension = ".pptx" Then
                    ...
                    'Directory.CreateDirectory("C:\SISTEMA ZITRO\COTI\" & n_carp)
                    File.Copy(OpenFileDialog1.FileName, Form_Reg_SRV_SQL.NOM_CARP_RQ & n_carp & "\" & nombrefinal)
