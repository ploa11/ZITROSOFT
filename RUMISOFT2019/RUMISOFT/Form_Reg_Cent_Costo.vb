Public Class Form_Reg_Cent_Costo
    ...
    Private Sub item()
        ...
        End If
        MessageBox.Show("Guarde el registro", "ZITRO")
            ...
            For o = 0 To ListView1.Items.Count - 1
                If accion = "guardar" Then
                    ...
                    Else
                        ...
                        sql = "exec alta_centrocosto '" + UCase(ListView1.Items(o).SubItems(1).Text) + "','" + UCase(ListView1.Items(o).SubItems(2).Text) + "','" + DateTimePicker1.Value.ToString("yyyyMMdd") + "','" + UCase(ListView1.Items(0).SubItems(4).Text) + "','" + UCase(ListView1.Items(0).SubItems(5).Text) + "'"
                        Form_Reg_SRV_SQL.conectar()
                            com = New SqlClient.SqlCommand(sql, Form_Reg_SRV_SQL.conexion)
    ...
    Private Sub llenar_grid()
        Try
            sql = "select COD AS [CODIGO],COD_CLIE AS [CODIGO DE CLIENTE], COD_SEDE AS [CODIGO DE SEDE] , FEC_REG AS [FECHA DE REGISTRO], NOM_CLIE AS [NOMBRE DE CLIENTE],NOM_SEDE AS [NOMBRE DE SEDE] from T_CENTRO_COSTO"
            Form_Reg_SRV_SQL.conectar()
            da = New SqlClient.SqlDataAdapter(sql, Form_Reg_SRV_SQL.conexion)
            cb = New SqlClient.SqlCommandBuilder(da)
            ds = New DataSet
